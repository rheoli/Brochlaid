...
package body pnumber is

  protected body ptoNumber is
    procedure start ( x : in integer; y : in integer; s : in integer; ks : in integer; m : in integer; d : in integer ) is
    begin
      fStarted            := true;
      fFocus              := false;
      iX                  := x;
    ...
    begin
      ...
      if ( fFocus ) then
        poScreen.paintInversWindow ( iX, iY-1, iAnzStellen+iWidth, 3, "" );
        poScreen.putStringXY ( iX+1, iY, strNumber(1..(iAnzStellen-iKommaStelle)) );
        if ( iKommaStelle > 0 ) then
          poScreen.putCharXY ( iX+(iAnzStellen-iKommaStelle)+1, iY, CHARACTER'POS('.') );
          poScreen.putStringXY ( iX+(iAnzStellen-iKommaStelle)+2, iY, strNumber((iAnzStellen-iKommaStelle+1)..iAnzStellen) );
        end if;
        if ( iPos <= (iAnzStellen-iKommaStelle) ) then
          poScreen.putInversCharXY ( iX+iPos, iY, CHARACTER'POS(strNumber(iPos)) );
        else
          poScreen.putInversCharXY ( iX+iPos+1, iY, CHARACTER'POS(strNumber(iPos)) );
        end if;
      else
        poScreen.paintWindow ( iX, iY-1, iAnzStellen+iWidth, 3, "" );
        poScreen.putStringXY ( iX+1, iY, strNumber(1..(iAnzStellen-iKommaStelle)) );
        if ( iKommaStelle > 0 ) then
        poScreen.putCharXY ( iX+(iAnzStellen-iKommaStelle)+1, iY, CHARACTER'POS('.') );
          poScreen.putStringXY ( iX+(iAnzStellen-iKommaStelle)+2, iY, strNumber((iAnzStellen-iKommaStelle+1)..iAnzStellen) );
